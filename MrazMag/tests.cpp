...
TEST_CASE("First client arrive later than 0 minute") {
	TestStore store;
	store.init(2, 10, 0);
	...
	SECTION("Nothing must happend") {
		store.advanceTo(0);

		INFO("no clients in the store at min 0");
		REQUIRE(store.log.size() == 0);
	}
...
TEST_CASE("One supplier equal need") {
	TestStore store;
	store.init(1, 0, 0);
	...
	SECTION("sent supplier for Banana but not for schweppes") {
		store.advanceTo(0);

		INFO("clients arrive at 0 min and supplier is sent for Banan");
		REQUIRE(store.log.size() == 1);
		REQUIRE(LastEvent().type == StoreEvent::WorkerSend);
	...
	SECTION("One worker must be sent") {
		...
		INFO("First client depart with nothing");
		REQUIRE(store.log.size() == 2);
		REQUIRE(LastEvent().type == StoreEvent::ClientDepart);
		REQUIRE(LastEvent().client.banana == 0);
		REQUIRE(LastEvent().client.schweppes == 0);
...
TEST_CASE("Two supplier") {
	...
	SECTION("Two suppliers must be sent") {
		store.advanceTo(0);

		INFO("clients arrive at 0 min and two suppliers are sent ");
		REQUIRE(store.log.size() == 2);
		REQUIRE(LastEvent().type == StoreEvent::WorkerSend);
		REQUIRE(LastEvent().worker.resource == ResourceType::schweppes);
	}

	SECTION("No suppliers must be sent") {
		store.advanceTo(30);

		INFO("Client arrive at 30 min but there is no free suppliers");
		REQUIRE(store.log.size() == 2);
	}

	SECTION("Two deliveries , One client depart") {
		store.advanceTo(60);

		INFO("Two deliveries , First client depart ");
	...
	SECTION("Second client") {
		...
		INFO("second client departs with nothing");
...
TEST_CASE("All clients arrive at the same time") {
	TestStore store;
	store.init(4, 0, 0);
	...
	SECTION("Two suppliers must be sent") {
		...
		INFO("client arrive at 0 min and two suppliers are sent ");
		REQUIRE(store.log.size() == 2);
		REQUIRE(LastEvent().type == StoreEvent::WorkerSend);
		REQUIRE(LastEvent().worker.resource == ResourceType::schweppes);
	}
	...
	SECTION("First client depart") {
		...
		INFO("2 Deliveries , First client depart with all he wants");
		REQUIRE(store.log.size() == 5);
		REQUIRE(LastEvent().type == StoreEvent::ClientDepart);
		REQUIRE(LastEvent().client.index == 0);
	}
	...
	SECTION("Second client depart") {
		...
		INFO("Second client depart with nothing");
		REQUIRE(store.log.size() == 6);
		REQUIRE(LastEvent().type == StoreEvent::ClientDepart);
		REQUIRE(LastEvent().client.index == 1);
	}

	SECTION("Third client depart") {
		store.advanceTo(130);

		INFO("Third client depart only with schweppes");
